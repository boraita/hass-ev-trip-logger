...
| **Energy precision** | Effective battery capacity auto-calibrated from real charges (median of `kwh / ΔSoC × 100` over recent charges ≥ 30 % ΔSoC). Since v0.8.14, prefers charges whose `kwh` came from a coverage-verified power-integration measurement over the SoC-delta guess it exists to correct, once ≥5 of those exist. Heals all historical SoC-derived trips when calibration shifts. |
...
- **Power-integration during charge (v0.5.89+, coverage-aware since v0.8.14)** — if your `power_sensor` is wired, the integration sums ∫P·dt over the whole charge. Captures the full charge curve including the high-SoC taper where each percent represents more time → more accurate than `Δ% × nominal_capacity`, especially on short/away-from-home top-ups where ±1 % SoC quantization dominates the signal. This measurement is only trusted once it's actually earned it: it needs ≥3 power samples spanning at least half the session, otherwise a cloud dropout mid-charge (common on a fast public DCFC session, where the vehicle's cloud API may only report the initial power spike) would silently under-count the real energy while looking numerically plausible. With good coverage it's accepted within ±40 % of the SoC-derived number; without it, the SoC math is used regardless of how close the sparse reading happens to look. Each charge records which method won as `energy_source` (`power_integration` / `soc_delta` / `manual` for a hand-corrected value) — visible as an attribute on `sensor.<device>_recent_charges` / `sensor.<device>_last_charge_kwh`. The battery-capacity calibration below now prefers `power_integration`-tagged charges once ≥5 exist, instead of partly re-deriving capacity from its own SoC-delta guess.
...
- **Automatic AC replay when the live integral is empty (v0.8.19)** — the live integration only accumulates while a charge session object exists, and with a cloud-polled `charge_sensor` the session is sometimes opened (or reconstructed) after the wallbox already finished. Those charges used to close with `evse_energy_kwh` NULL — no AC side, no efficiency — even though the wallbox had reported perfectly. Now, when a home charge closes (or a pulse merges into one) with nothing in the live integral, the integration replays the session window from the recorder 30 s later: the same integration the manual [`backfill_charge_evse`](#services) service does, masked by `charge_sensor` so the wallbox's standby draw between pulses isn't counted. Only fires with an `evse_power_sensor` configured and at a home location, and a pending replay is dropped if the entry reloads.
